...
function settingsCandidates() {
  ...
  return [
    path3.join(home, ".config", "zed", "settings.json"),
    path3.join(home, "Library", "Application Support", "Zed", "settings.json")
  ];
}
async function readZedInitializationOptions(serverId = "ai-lens") {
  for (const candidate of settingsCandidates()) {
    try {
      const raw = await fs2.readFile(candidate, "utf-8");
...
async function latestEntry(cacheDir) {
  const metaDir = path4.join(cacheDir, "meta");
  let names;
  try {
    names = (await fs3.readdir(metaDir)).filter((n) => n.endsWith(".json"));
  } catch {
    return null;
  }
  let best = null;
  for (const name of names) {
    try {
      const meta = JSON.parse(await fs3.readFile(path4.join(metaDir, name), "utf-8"));
      if (meta.skipped)
        continue;
  ...
  if (!best)
    return null;
  try {
    const markdown = await fs3.readFile(cachePaths(cacheDir, best.source).outPath, "utf-8");
      ...
      try {
        ...
        if (url === "/latest") {
          ...
          res.end(JSON.stringify({
            id: found.meta.key,
            title: path4.basename(found.meta.source),
...
async function main() {
  ...
  if (!target)
    usage();
  const filePath = path5.resolve(target);
  const fileName = path5.basename(filePath);
  let content;
  try {
    content = await fs4.readFile(filePath, "utf-8");
  } catch (error) {
    console.error(`ai-lens: cannot read ${filePath}: ${error.message}`);
    process.exit(1);
  }
...
`;
    ...
    await writeOutput(entry, produced, { key, source: filePath, agent: config.agent, model: config.model }, config.readOnlyOutput);
...
async function isPreviewUp(url) {
  try {
...
function open(url) {
  ...
  try {
    import_child_process3.spawn(opener, [url], { detached: true, stdio: "ignore" }).unref();
  } catch {}
}
main().catch((error) => {
